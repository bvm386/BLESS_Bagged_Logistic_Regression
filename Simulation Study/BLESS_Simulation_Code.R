...
for (param_index in 1:nrow(parameter_combinations)) {
  ...
  for(jj in 1:100){
    ...
    real.negative<-length(which(!colnames(refined.SNP.set) %in% top.30.SNP$SNPs))
    
    pred.positive<-length(adj.selected)
    pred.negative<-length(which(!colnames(refined.SNP.set) %in% adj.selected))
    ...
    metrics$accuracy[jj]<- ((true.positive+true.negative)/dim(refined.SNP.set)[2])
